...
export async function getStaticProps({ params }) {
  const [data_sensorRes, deviceRes, data_yesterdayRes, data_listRes] =
    await Promise.all([
      fetch(`${process.env.HOST_NAME}/api/data-sensor/single/${params.pid}`),
      fetch(`${process.env.HOST_NAME}/api/device/${params.pid}`),
      fetch(`${process.env.HOST_NAME}/api/data-sensor/list/${params.pid}`),
      fetch(
        `${process.env.HOST_NAME}/api/data-sensor/list/${params.pid}?startDate=0&endDate=7`
      ),
    ]);
